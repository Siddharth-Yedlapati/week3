...
const encrypt = async (
  ...
  const iv = buf2Bigint(mimc7.multiHash(sharedKey, BigInt(0)));
  
  const ciphertext: Ciphertext = {
  iv, data: plaintext.map((e: bigint, i: number): bigint => { return e + buf2Bigint(mimc7.hash(sharedKey, iv + BigInt(i)));
  }),	
};
...
const decrypt = async (
  ...
  const plaintext: Plaintext = ciphertext.data.map(
  	(e: bigint, i: number): bigint => {
  		return (
	  		BigInt(e) - buf2Bigint(mimc7.hash(sharedKey, BigInt(ciphertext.iv) + BigInt(i)))
	  	);
},
);
return plaintext;
